...
class Magento19 extends \LitExtension\CartMigration\Model\Cart
{
    ...
    public function displayImport() {
        ...
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            'query' => serialize(array(
                ...
                'categories' => "SELECT COUNT(1) FROM _DBPRF_catalog_category_entity WHERE entity_id > {$this->_notice['categories']['id_src']} AND level > 1",
                'products' => "SELECT COUNT(1) FROM _DBPRF_catalog_product_entity WHERE entity_id > {$this->_notice['products']['id_src']}",
				//'products' => "SELECT COUNT(1) FROM _DBPRF_eav_attribute WHERE attribute_id > {$this->_notice['products']['id_src']} AND entity_type_id = 4",
                'customers' => "SELECT COUNT(1) FROM _DBPRF_customer_entity WHERE entity_id > {$this->_notice['customers']['id_src']}",
                'orders' => "SELECT COUNT(1) FROM _DBPRF_sales_flat_order WHERE entity_id > {$this->_notice['orders']['id_src']}",
                'reviews' => "SELECT COUNT(1) FROM _DBPRF_review WHERE review_id > {$this->_notice['reviews']['id_src']}",
    ...
    public function convertCategory($category, $categoriesExt) {
		...
		$long_description = $this->getListFromListByField($text, 'attribute_id', 941);
		//foreach($map as $store_src => $store_target){
			$this->insertTable('catalog_category_entity_text', array(
				'attribute_id' => 373,
				'store_id' => 0,//$store_target,
				'entity_id' => $this->getMageIdCategory($category['entity_id']),
				'value' => $this->getRowValueFromListByField($long_description, 'store_id', 0, 'value')
			));
		//}
        return array(
            'result' => 'pass',         
    ...
    protected function _getProductsMainQuery() {
		...
		//$query = "SELECT * FROM _DBPRF_eav_attribute WHERE entity_type_id = 4 AND attribute_id > {$id_src} ORDER BY attribute_id ASC LIMIT {$limit}";
        return $query;
    }
    ...
    public function getProductsExt($products) {
		...
		//$attribute_id = $this->duplicateFieldValueFromList($products['object'], 'attribute_id');
		//$attribute_id_query = $this->arrayToInCondition($attribute_id);
        $ext_query = array(
			//'eav_attribute_option' => "SELECT * FROM _DBPRF_eav_attribute_option WHERE attribute_id IN {$attribute_id_query}",
		...
		//$optionAttrIds = $this->duplicateFieldValueFromList($productsExt['object']['eav_attribute_option'], 'option_id');
        //$option_attr_id = $this->arrayToInCondition($optionAttrIds);
        $ext_rel_query = array(
			//'eav_attribute_option_value' => "SELECT * FROM _DBPRF_eav_attribute_option_value WHERE option_id IN {$option_attr_id}",
    ...
    public function convertProduct($product, $productsExt) {
		...
		/**
		$attribute = array();
		$product_mage_id = $this->getMageIdProduct($product['entity_id']);
		...
		$descriptions_dk = $this->getRowFromListByField($descriptions, 'store_id', 3);
		if($descriptions_dk){
			$this->insertTable('catalog_product_entity_text', array(
				'attribute_id' => 75,
				'store_id' => 9,
				'entity_id' => $product_mage_id,
				'value' => $descriptions_dk['value']
			));
		}
		$short_descriptions_dk = $this->getRowFromListByField($short_descriptions, 'store_id', 3);
		if($short_descriptions_dk){
			$this->insertTable('catalog_product_entity_text', array(
				'attribute_id' => 76,
				'store_id' => 9,
				'entity_id' => $product_mage_id,
				'value' => $short_descriptions_dk['value']
			));
		}
		$meta_titles_dk = $this->getRowFromListByField($meta_titles, 'store_id', 3);
		if($meta_titles_dk){
			$this->insertTable('catalog_product_entity_varchar', array(
				'attribute_id' => 84,
				'store_id' => 9,
				'entity_id' => $product_mage_id,
				'value' => $meta_titles_dk['value']
			));
		}
		$meta_keywords_dk = $this->getRowFromListByField($meta_keywords, 'store_id', 3);
		if($meta_keywords_dk){
			$this->insertTable('catalog_product_entity_text', array(
				'attribute_id' => 85,
				'store_id' => 9,
				'entity_id' => $product_mage_id,
				'value' => $meta_keywords_dk['value']
			));
		}
		$meta_descriptions_dk = $this->getRowFromListByField($meta_descriptions, 'store_id', 3);
		if($meta_descriptions_dk){
			$this->insertTable('catalog_product_entity_varchar', array(
				'attribute_id' => 86,
				'store_id' => 9,
				'entity_id' => $product_mage_id,
				'value' => $meta_descriptions_dk['value']
			));
		}
		*/
		/**
		$varchar = $this->getListFromListByField($productsExt['object']['catalog_product_entity_varchar'], 'entity_id', $product['entity_id']);
		$names = $this->getListFromListByField($varchar, 'attribute_id', $attribute['name']);
		$names_dk = $this->getRowFromListByField($names, 'store_id', 3);
		if($names_dk){
			$this->insertTable('catalog_product_entity_varchar', array(
				'attribute_id' => 73,
				'store_id' => 9,
				'entity_id' => $product_mage_id,
				'value' => $names_dk['value']
			));
		}
		
		$stock = $this->getRowFromListByField($productsExt['object']['cataloginventory_stock_item'], 'product_id', $product['entity_id']);		
		$this->updateTable('cataloginventory_stock_item',array(
		...
		//$attribute_label = $this->getListFromListByField($productsExt['object']['eav_attribute_label'], 'attribute_id', $product['attribute_id']);		
		
		/**$attrDm = $this->getRowFromListByField($attribute_label, 'store_id', 3);
			if($attrDm){
				$this->insertTable('eav_attribute_label',array(
					'attribute_id' => $attribute_desc['attribute_id'],
					'store_id' => 9,
					'value' => $attrDm['value']
				));
			}
		$msg = '<br/>Attribute : ' . $product['attribute_code'] . ' - Updated label!';
		*/
		
		
		$decimal = $this->getListFromListByField($productsExt['object']['catalog_product_entity_decimal'], 'entity_id', $product['entity_id']);
		$price = $this->getListFromListByField($decimal, 'attribute_id', 60);
		$special_price = $this->getListFromListByField($decimal, 'attribute_id', 61);
		$map = array(
			7 => 7,
			8 => 8,
			9 => 2,
        ...
        foreach($map as $store_src => $store_target){
			$this->insertTable('catalog_product_entity_decimal', array(
				'attribute_id' => 78,
				'store_id' => $store_target,
				'entity_id' => $this->getMageIdProduct($product['entity_id']),
				'value' => $this->getRowValueFromListByField($special_price, 'store_id', $store_src, 'value')
			));
		}
        return array(
            'result' => 'pass',
	...
	public function selectOptionValueDesc($attribute_id, $value_store0){       
		$query = "SELECT * FROM `eav_attribute_option` as eao LEFT JOIN `eav_attribute_option_value` as eaov ON eaov.option_id = eao.option_id
					WHERE eao.`attribute_id` = ".$attribute_id." AND eaov.`value` = '".$value_store0."' AND eaov.store_id = 0";
        try{
